"""
Pull & Bear Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Pull & Bear.

Docs  : https://docs.bright.cn/scraping-automation/web-scraper-api/overview
Info  : https://bright.cn/products/insights/price-tracker/pull-and-bear
"""
...
# Search for "Pull & Bear" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Navigate to: https://bright.cn/cp/scrapers\n"
        "2. Search for 'Pull & Bear' and copy the Web Scraper ID.\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Pull & Bear product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Pull & Bear products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Pull & Bear category page.

    Example:
        results = discover_by_category("https://pullandbear.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.pullandbear.com/en/products/sample-product-123456",
        # Add more Pull & Bear product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "pull-and-bear_prices.json"
    ...
    # cat_results = discover_by_category("https://pullandbear.com/category/sample", limit=50)
